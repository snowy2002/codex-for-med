...
fn reasoning_effort_mapping_from_presets(
    ...
    // Map every canonical effort to the closest supported effort for the new model. Model-defined
    // custom values are targets, not canonical source values, so they do not become migration keys.
...
fn canonical_reasoning_efforts() -> [ReasoningEffort; 8] {
    [
...
fn effort_rank(effort: &ReasoningEffort) -> Option<i32> {
    match effort {
...
fn nearest_effort(target: &ReasoningEffort, supported: &[ReasoningEffort]) -> ReasoningEffort {
    ...
    let Some(target_rank) = effort_rank(target) else {
        return supported.first().cloned().unwrap_or_else(|| target.clone());
    };
    supported
        .iter()
        .filter_map(|candidate| Some((candidate, effort_rank(candidate)?)))
        .min_by_key(|(_, candidate_rank)| (*candidate_rank - target_rank).abs())
...
mod tests {
    ...
    fn reasoning_effort_from_str_preserves_unknown_values() {
        ...
        assert_eq!(
            serde_json::to_string(&ReasoningEffort::Custom("future".to_string())).unwrap(),
            "\"future\""
        );
    }

    #[test]
    fn reasoning_effort_from_str_rejects_empty_value() {
        assert_eq!(
